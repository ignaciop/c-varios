...
int main(int argc, char* argv[]) {
    ...
    struct requests_queue *requests = init_requests_queue(&request_mutex, &got_request);
    assert(requests);

    /* create the handler threads list */
    struct handler_threads_pool *handler_threads = init_handler_threads_pool(&request_mutex, &got_request, requests);
    ...
    for (int i = 0; i < 600; i++) {
	    ...
	    int num_requests = get_requests_number(requests); // number of requests waiting to be handled.
	    int num_threads = get_handler_threads_number(handler_threads); // number of active handler threads.
	    ...
	    /* a handler thread. */
	    if (num_requests > HIGH_REQUESTS_WATERMARK && num_threads < MAX_NUM_HANDLER_THREADS) {
