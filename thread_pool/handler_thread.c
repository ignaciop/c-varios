...
void *handle_requests_loop(void *thread_params) {
    /* handler thread's parameters */
    struct handler_thread_params *data = (struct handler_thread_params *)thread_params;
    ...
    int rc = pthread_mutex_lock(data->request_mutex);
    ...
    while (1) {
	    ...
	    if (num_requests > 0) { /* a request is pending */
	        ...
	        if (a_request != NULL) { /* got a request - handle it and free it */
